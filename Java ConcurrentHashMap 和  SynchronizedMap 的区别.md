...
- 提供 putIfAbsent、computeIfAbsent 和 replace 等原子方法进行复合操作，避免手动同步的复杂性
...
## 总结
- 迭代器：ConcurrentHashMap 是弱一致性的，迭代过程中允许其他线程修改 Map，不会抛出异常，但迭代器可能不会反映最新的修改（仅保证迭代时数据操作是安全的），而 SynchronizedMap 在迭代过程中如果被其他线程修改时（比如 put 或 remove），会立即抛出异常
